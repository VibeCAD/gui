      ...
      try {
        ...
        if (command.startsWith('create ')) {
          ...
          if (parts.length >= 3) {
            ...
            // Log the Babylon.js code that will be executed
            const babylonCode = `
// Babylon.js code generated by MCP:
const ${parts[2]} = MeshBuilder.Create${parts[1].charAt(0).toUpperCase() + parts[1].slice(1)}('${parts[2]}', {
  ${parts[1] === 'box' ? 'size' : 'diameter'}: ${params?.size || 2}
}, scene);
${parts[2]}.position = new Vector3(${params?.position?.x || 0}, ${params?.position?.y || 1}, ${params?.position?.z || 0});
const ${parts[2]}Material = new StandardMaterial('${parts[2]}-material', scene);
${parts[2]}Material.diffuseColor = Color3.FromHexString('${params?.color ? rgbToHex(params.color) : '#3498db'}');
${parts[2]}.material = ${parts[2]}Material;
`
            console.log('🎮 Generated Babylon.js code:', babylonCode)
          }
    ...
    try {
      switch (command.action) {
        ...
        case 'create':
          ...
          // Log the actual Babylon.js execution
          console.log('🔨 Executing Babylon.js creation for:', command.type, 'with ID:', newId)
          
          if (command.type === 'cube' || command.type === 'box') {
            console.log(`🎯 MeshBuilder.CreateBox('${newId}', { size: ${command.size || 2} }, scene)`)
            newMesh = MeshBuilder.CreateBox(newId, { size: command.size || 2 }, scene)
          } else if (command.type === 'sphere') {
            console.log(`🎯 MeshBuilder.CreateSphere('${newId}', { diameter: ${command.size || 2} }, scene)`)
            newMesh = MeshBuilder.CreateSphere(newId, { diameter: command.size || 2 }, scene)
          } else if (command.type === 'cylinder') {
            console.log(`🎯 MeshBuilder.CreateCylinder('${newId}', { diameter: ${command.size || 2}, height: ${command.height || 2} }, scene)`)
          ...
          console.log(`🎯 ${newId}.position = new Vector3(${command.x || 0}, ${command.y || 1}, ${command.z || 0})`)
          ...
          console.log(`🎯 ${newId}Material.diffuseColor = Color3.FromHexString('${command.color || '#3498db'}')`)
          ...
          console.log('✨ Complete Babylon.js object created:', {
            id: newId,
            type: command.type,
            babylonMesh: newMesh,
            position: `(${command.x || 0}, ${command.y || 1}, ${command.z || 0})`,
            color: command.color || '#3498db'
          })
